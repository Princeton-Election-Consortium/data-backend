...
assignedEV(1)=45; assignedEV(2)=43; % these are the seats not up for election or safe
...
todayte=floor(today-datenum('31-dec-2021'));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
polldata=load('2022.Senate.polls.median.txt');
% column 1 - numpolls
% column 2 - lastdate
...
if mod(numlines,num_states)>0
    warning('Warning: polls.median.2022Senate.txt is not a multiple of num_states lines long');
end
...
%    print -djpeg Senate_histogram_today.jpg
     screen2jpeg(['Senate_histogram_today_2022.jpg'])
end
%
% end plot
%
...
%   Export probability histogram:
    dlmwrite(strcat(whereoutput,'Senate_histogram_2022.csv'),histogram')
...
if exist(strcat(whereoutput,'Senate_stateprobs_2022.csv'),'file')
    delete(strcat(whereoutput,'Senate_stateprobs_2022.csv'))
end
...
for ii=1:num_states
        ...
        dlmwrite(strcat(whereoutput,'Senate_stateprobs_2022.csv'),foo,'-append','delimiter','')
    end
end
...
dlmwrite(strcat(whereoutput,'Senate_estimates_2022.csv'), outputs) % just today's estimate
if forhistory
   dlmwrite(strcat(whereoutput,'Senate_estimate_history_2022.csv'),[polldata(1,5) outputs],'-append')
end
