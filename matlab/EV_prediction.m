...
% Sam will return to this logic
M2016=mean_MM(strcat(whereoutput,EV_ESTIMATE_HISTORY_CSV),3.5); % starting 8/19, calculate this from time series for 2016. LAST UPDATE: July 18 2020
...
EV_MM_table=load(strcat(whereoutput,EV_MM_TABLE_CSV));
mmf=EV_MM_table(:,1);
evf=EV_MM_table(:,2);
...
dlmwrite(strcat(whereoutput,EV_PREDICTION_CSV), outputs)

%% write probabilities to csv
outputs = [ bayesian_winprob drift_winprob ];
dlmwrite(strcat(whereoutput,EV_PREDICTION_PROBS_CSV), outputs)

%% write meta-margin prediction to csv for plotter scripts
outputs = [ Msig1lo Msig1hi Msig2lo Msig2hi ];
dlmwrite(strcat(whereoutput,EV_PREDICTION_MM_CSV), outputs)
