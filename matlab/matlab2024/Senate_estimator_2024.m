...
%%%  Copyright 2008, 2016, 2020, 2022 by Samuel S.-H. Wang
...
% Senate_estimator.m
% 
% This script loads '2022.Senate.polls.median.txt' and generates or updates/replaces 4 CSV files:
% 
...
polls.state=['AZ,FL,MD,MI,MT,NV,OH,PA,TX,WI,WV ']; % 11 races.
contested=[1:length(polls.state)/3]; % races in serious question
polls.EV=ones(1, length(polls.state)/3);
num_states=size(polls.EV,2);

assignedEV(3)=sum(polls.EV);
assignedEV(1)=42; assignedEV(2)=47; % these are the seats not up for election or safe
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
polldata=load('2024.Senate.polls.median.txt');
% column 1 - numpolls
% column 2 - lastdate
...
if mod(numlines,num_states)>0
    warning('Warning: polls.median.2024.Senate.txt is not a multiple of num_states lines long');
end
...
if numel(confidenceintervals(3)) == numel(Senateseats(find(cumulative_prob<=0.025,1,'last')))
    confidenceintervals(3)=Senateseats(find(cumulative_prob<=0.025,1,'last')); % 95-pct lower limit
end
if numel(confidenceintervals(1)) == numel(Senateseats(find(cumulative_prob<=0.15865,1,'last')))
    confidenceintervals(1)=Senateseats(find(cumulative_prob<=0.15865,1,'last')); % 1-sigma lower limit
end
if numel(confidenceintervals(2)) == numel(Senateseats(find(cumulative_prob>=0.84135,1,'first')))
...
for i=1:max(size(uncertain))
    uncertainstates=[uncertainstates statename(uncertain(i),polls.state) ' '];
end
...
%   Export probability histogram:
    dlmwrite(strcat(whereoutput,'Senate_histogram_2024.csv'),histogram')
...
daystoelection=datenum('08-Nov-2022')-today; % days until election (note: November 8, Julian 314)
...
if exist(strcat(whereoutput,'Senate_stateprobs_2024.csv'),'file')
    delete(strcat(whereoutput,'Senate_stateprobs_2024.csv'))
end
...
for ii=1:num_states
        foo=[num2str(stateprobs(ii)) ',' num2str(statenovprobs(ii)) ',' num2str(polls.margin(ii)) ',' num2str(D2probs(ii)) ',' num2str(R2probs(ii)) ',' statename(ii,polls.state)];
        dlmwrite(strcat(whereoutput,'Senate_stateprobs_2024.csv'),foo,'-append','delimiter','')
    end
end
...
dlmwrite(strcat(whereoutput,'Senate_estimates_2024.csv'), outputs) % just today's estimate
if forhistory
   dlmwrite(strcat(whereoutput,'Senate_estimate_history_2024.csv'),[polldata(1,6) outputs],'-append')
end
